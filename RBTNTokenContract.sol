...
contract orbitaintoken {
    ...
    uint public openTime = 1651843200;
    uint public startTime = 1651843800;
    ...
    constructor()  {
        _mint(owner, 150000000 * 10 ** decimals());//mint 200 million token to owner address
    }
    ...
    function contribute(uint slotNumber) public payable {
        require(msg.value >= 0.05 ether, "Minimum Contribution is 0.05 ETH");
    ...
    function burnToRegister(uint256 amount, string memory publicKey) public {
    ...
    function _mint(address account, uint256 amount) internal virtual {
        require (totalMinted <= totalSupply, "RBTN: Reached Totalsupply");
        require (account != address(0), "RBTN: mint to zero account");
    ...
    function _burn(address account, uint256 amount) internal virtual {
        require(account != address(0), "RBTN: burn from the zero address");

        _beforeTokenTransfer(account, address(0), amount);

        uint256 accountBalance = balances[account];
        require(accountBalance >= amount, "RBTN: burn amount exceeds balance");
        unchecked {
    ...
    function _transfer(
        ...
        require(from != address(0), "RBTN: transfer from the zero address");
        require(to != address(0), "RBTN: transfer to the zero address");

        _beforeTokenTransfer(from, to, amount);

        uint256 fromBalance = balances[from];
        require(fromBalance >= amount, "RBTN: transfer amount exceeds balance");
        unchecked {
